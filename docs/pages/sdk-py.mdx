...
In an **async** context, you can use the `EmbedbaseClientAsync` class instead.
This class provides the same methods as `EmbedbaseClient`, but they are all asynchronous.
```python
from embedbase_client.client import EmbedbaseClientAsync
```

Remember to use `await` when calling methods on `EmbedbaseClientAsync` objects.
