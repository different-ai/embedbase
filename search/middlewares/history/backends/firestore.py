...
class FirestoreBackend(BaseBackend):
    ...
    def __init__(
        ...
        if not asyncio.iscoroutinefunction(can_log):
            raise ValueError(f"invalid can_log function, it must be a coroutine: {can_log}")
    ...
    async def can_log(self, user: str, group: str, scope: Scope) -> Optional[str]:
        if not self._can_log:
            return None
        return await self._can_log(self._firestore, user, group, scope)
