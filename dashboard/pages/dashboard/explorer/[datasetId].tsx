  ...
  const handleUpdateDocument = async () => {
    ...
    if (newContent) {
      ...
      if (res.error) {
        ...
        toast.success(
          'Document updated successfully, refresh to see the difference'
        )
      }
    }
        ...
        <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
          ...
          >
            <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-sm sm:p-6">
              <div>
                <textarea
                  className="mt-4 h-40 w-full resize-none rounded"
              ...
              <div className="mt-5 sm:mt-6">
                <div className="flex justify-end gap-2">
                  <button
                    type="button"
                    className="focus-ring-2 focus-ring-offset-2 inline-flex items-center rounded-md border border-transparent bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-200 focus:outline-none focus:ring-purple-500"
                  ...
                  <button
                    type="button"
                    className={`focus-ring-2 focus-ring-offset-2 inline-flex items-center rounded-md border border-transparent bg-purple-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-purple-700 focus:outline-none focus:ring-purple-500 ${
...
const DataTable = ({
  ...
  const onShareDataset = async () => {
    ...
    if (!res.data.username) {
      ...
      toast.error(
        'You need to set a username before sharing datasets. Redirecting you to account page'
      )
    ...
    console.log(
      `making dataset ${userId}:${datasetId} ${
    ...
    console.log(
      `dataset ${datasetId} is now ${!isPublic ? 'public' : 'private'}`
    )
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex flex-col items-end justify-end">
          ...
          <div className="hidden text-right text-xs text-gray-500 sm:block">
            {isPublic ? (
              <p>
                This dataset is currently public. Anyone on the internet can
                read and search this dataset.
                <br /> Only you can add and write to this dataset.
              </p>
            ) : (
              <p>
                This dataset is currently private.
                <br /> Only you can read and write to this dataset.
              </p>
            )}
          </div>
        </div>
          ...
          {documents.map((document, index) => (
            <Fragment key={document.id}>
              ...
              >
                ...
                <td
                  className="cursor-context-menu select-none px-4 py-4 text-xs text-gray-500"
                  onClick={() => handleCopyToClipboard(document.id)}
                >
                  {document.id.slice(0, 10)}
                </td>
                <td>
                  <div className="max-h-[100px] select-none px-3 py-3.5 text-left text-sm text-gray-900">
              ...
              {activeDocument && activeDocument.id === document.id && (
                <tr onDoubleClick={() => handleDoubleClick(null)}>
                  <td colSpan={2}>
                    ...
                    {/* <Markdown> */}
                    <div className="max-w-[80%] overflow-auto px-3 py-3.5 text-left text-sm text-gray-900">
                      {activeDocument.data}
                      {/* </Markdown> */}
                    </div>
                    {/* </div> */}
