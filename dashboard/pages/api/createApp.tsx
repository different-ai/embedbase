...
const handler = async (req: Request, res: Response): Promise<Response> => {
    ...
    if (!systemMessage) {
        return new Response(JSON.stringify({ error: 'No system message provided' }), {
            status: 401,
        })
    }
    ...
    const { data, error } = await supabase
        .from('apps')
        .insert({ owner: userId, name: name, datasets: datasets, system_message: systemMessage, publicApiKey: uuid })
    ...
    return new Response(JSON.stringify({ link: `${baseUrl}chat?appId=${uuid}` }), {
        status: 200,
    })
}
